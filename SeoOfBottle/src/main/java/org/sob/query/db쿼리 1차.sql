...
CREATE TABLE IF NOT EXISTS `boast` (
  `boastid` int(20) NOT NULL AUTO_INCREMENT COMMENT '자랑게시판인덱스',
  `groupid` varchar(100) NOT NULL DEFAULT '0' COMMENT '그룹아이디',
...
CREATE TABLE IF NOT EXISTS `bottlemain` (
  ...
  `from` varchar(50) NOT NULL DEFAULT '0' COMMENT '보낸사람',
  `to` varchar(50) NOT NULL DEFAULT '0' COMMENT '받는사람',
  ...
  KEY `FK_bottlemain_boast` (`groupid`),
  CONSTRAINT `FK_bottlemain_boast` FOREIGN KEY (`groupid`) REFERENCES `boast` (`groupid`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='기본적인 rwud 테이블';
