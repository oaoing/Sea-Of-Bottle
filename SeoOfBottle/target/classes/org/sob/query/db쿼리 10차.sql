...
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8 COMMENT='자랑게시판 리플관련 테이블';
...
CREATE TABLE IF NOT EXISTS `bottle` (
  ...
  CONSTRAINT `FK_bottle_category` FOREIGN KEY (`categoryid`) REFERENCES `category` (`categoryid`)
) ENGINE=InnoDB AUTO_INCREMENT=46 DEFAULT CHARSET=utf8 COMMENT='기본적인 rwud 테이블';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='편지테이블';

-- 내보낼 데이터가 선택되어 있지 않습니다.
-- 테이블 seaofbottle.recievertable 구조 내보내기
CREATE TABLE IF NOT EXISTS `recievertable` (
  ...
  CONSTRAINT `FK_recievertable_customer` FOREIGN KEY (`receiver`) REFERENCES `customer` (`customerno`),
  CONSTRAINT `FK_recievertable_bottle` FOREIGN KEY (`labelid`) REFERENCES `bottle` (`labelid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='편지받은 사람 목록';
